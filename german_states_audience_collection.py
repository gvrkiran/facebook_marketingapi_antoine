...
if len(sys.argv) != 9:
    print("ERROR: You must enter 8 values: nb_interests, interest_file_num, start_state_nb, bhv, lang,"
          " age, gen, schol")
...
"""
states = pd.read_csv('german_states.csv')
state_count = 0

for state in states.values:
    # Compute the scores for each state
    if state_count >= start_state_nb:
        '''
        ...
        dic_target = {
            ...
            "geo_locations": [{"name": "regions",
                               "values": [{"key": state[0], "name": state[1], "country": "DE"}],
        ...
        '''
        json.dump(dic_target, open("./input_examples/" + time_stamp + "_" + state[1] + "_audience_collection.json", "w"),
                  indent=2, separators=(',', ': '))
        data_target = watcherAPI.run_data_collection("./input_examples/" + time_stamp + "_" + state[1] + "_audience_collection.json")
        ...
        # Save the file
        title = "./results/" + time_stamp + "_" + state[1] + "_" + str(interests['name'].shape[0]-1) + "_interests" \
